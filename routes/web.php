...
Route::get('/', function () {
    ...
    $wilayahData = collect(); // Default empty
    if (Schema::hasTable('penduduks')) {
        // --- Added Logic for Map Modal Data ---
        ...
        $buildCategoryStats = function (string $foreignKey, string $referenceTable) use ($deadStatusId) {
    ...
    $youtubeVideos = Schema::hasTable('youtube_videos')
        ? YoutubeVideo::published()->ordered()->take(8)->get()
        : collect();

    $sekilasInfos = Schema::hasTable('sekilas_infos')
        ? SekilasInfo::published()->ordered()->get()
        : collect();
    ...
    $publicInfoHours = Schema::hasTable('public_info_hours') && $publicInfoSetting->exists
        ? $publicInfoSetting->hours->map(function ($hour) {
    ...
    if ($heroSlides->isEmpty()) {
        ...
        $heroSlides = collect($fallback)->map(function ($slide) {
            return (object) $slide;
        });
    }

    $heroSlides = $heroSlides->map(function ($slide) {
        $imagePath = data_get($slide, 'background_url');
        $imageUrl = asset('img/Banner/gambar_desa_1.png');
